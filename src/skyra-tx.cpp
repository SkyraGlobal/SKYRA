...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " skyra-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  skyra-tx [options] <hex-tx> [commands]  Update hex-encoded skyra transaction\n" +
                               "or:     skyra-tx [options] -create [commands]   Create hex-encoded skyra transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded skyra transaction
