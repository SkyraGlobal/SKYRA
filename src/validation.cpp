...
#if defined(NDEBUG)
#error "SKYRA cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usky) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k SKY
    ...
    if (nHeight > 0 && nHeight < nLast) {
        ...
        return 5.75 * COIN;
    } else if (nHeight > 2500000 && nHeight <= 2699683) {
        return 7.70 * COIN;
    } else if (nHeight == 2699684) {
        return 3.40 * COIN;
    } else {
        return 0 * COIN;
    }
...
{
    util::ThreadRename("skyra-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // SKYRA Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nSKYRABadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nSKYRABadBlockBits)) {
            // accept SKYRA block minted with incorrect proof of work threshold
            return true;
        }
