...
#include "qt/skyra/defaultdialog.h"
#include "shutdown.h"
...
const QString SKYRAGUI::DEFAULT_WALLET = "~Default";

SKYRAGUI::SKYRAGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void SKYRAGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &SKYRAGUI::toggleHidden);
...
void SKYRAGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &SKYRAGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &SKYRAGUI::changeTheme);
    //connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &SKYRAGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &SKYRAGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &SKYRAGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &SKYRAGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &SKYRAGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &SKYRAGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &SKYRAGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &SKYRAGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &SKYRAGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &SKYRAGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &SKYRAGUI::execDialog);
}


void SKYRAGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void SKYRAGUI::handleRestart(QStringList args)
{
...
void SKYRAGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &SKYRAGUI::message);
...
void SKYRAGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &SKYRAGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &SKYRAGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void SKYRAGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void SKYRAGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void SKYRAGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &SKYRAGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &SKYRAGUI::show);
                e->ignore();
            }
        }
    }
...
void SKYRAGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void SKYRAGUI::messageInfo(const QString& text)
{
...
void SKYRAGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool SKYRAGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void SKYRAGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void SKYRAGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SKYRAGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void SKYRAGUI::goToSend()
{
    showTop(sendWidget);
}

void SKYRAGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void SKYRAGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void SKYRAGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void SKYRAGUI::goToGovernance()
{
    showTop(governancewidget);
}

void SKYRAGUI::goToSettings(){
    showTop(settingsWidget);
}

void SKYRAGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void SKYRAGUI::goToReceive()
{
    showTop(receiveWidget);
}

void SKYRAGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void SKYRAGUI::showTop(QWidget* view)
{
...
void SKYRAGUI::changeTheme(bool isLightTheme)
{
...
void SKYRAGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool SKYRAGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void SKYRAGUI::showHide(bool show)
{
...
int SKYRAGUI::getNavWidth()
{
    return this->navMenu->width();
}

void SKYRAGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void SKYRAGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void SKYRAGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool SKYRAGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &SKYRAGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &SKYRAGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &SKYRAGUI::message);
    connect(topBar, &TopBar::message, this, &SKYRAGUI::message);
    connect(sendWidget, &SendWidget::message,this, &SKYRAGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &SKYRAGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &SKYRAGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &SKYRAGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &SKYRAGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &SKYRAGUI::incomingTransaction);

    return true;
}

bool SKYRAGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void SKYRAGUI::removeAllWallets()
{
    // Single wallet supported.
}

void SKYRAGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(SKYRAGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SKYRAGUI::subscribeToCoreSignals()
{
...
void SKYRAGUI::unsubscribeFromCoreSignals()
{
