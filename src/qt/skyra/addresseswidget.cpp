...
#include "qt/skyra/forms/ui_addresseswidget.h"
#include "qt/skyra/addressfilterproxymodel.h"
#include "qt/skyra/addresslabelrow.h"
...
AddressesWidget::AddressesWidget(SKYRAGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto skyAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(skyAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(skyAdd);
        if (walletModel->updateAddressBookLabels(skyAdd, label.toUtf8().constData(),
