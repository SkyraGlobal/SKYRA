...
#include "qt/skyra/topbar.h"
#include "qt/skyra/forms/ui_topbar.h"
#include "qt/skyra/lockunlock.h"
#include "qt/skyra/qtutils.h"
#include "qt/skyra/receivedialog.h"
#include "qt/skyra/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/skyra/balancebubble.h"
...
TopBar::TopBar(SKYRAGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountSky}, "amount-topbar");
    setCssProperty({ui->labelPendingSky, ui->labelImmatureSky, ui->labelAvailableSky, ui->labelLockedSky, ui->labelStakingStatus,
                       ui->labelCollateralSky, ui->labelMNRewardvalue, ui->labelStakingRewardvalue},"amount-small-topbar");
    ...
    ui->pushButtonConf->setButtonText("skyra.conf");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SKYRA")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    if (g_tiertwo_sync_state.IsSynced()) {
        ui->labelCollateralSky->setText(GUIUtil::formatBalance(CMasternode::GetMNCollateral(chainActive.Tip()->nHeight) * COIN, nDisplayUnit));

        CAmount Blockvalue = GetBlockValue(chainActive.Tip()->nHeight);
        CAmount MNReward = Blockvalue * 0.98;

        ui->labelMNRewardvalue->setText(GUIUtil::formatBalanceTopBar(MNReward));
        ui->labelStakingRewardvalue->setText(GUIUtil::formatBalanceTopBar(Blockvalue - MNReward));
    }
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // SKY Total
    QString totalSky = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // Expanded
    ui->labelAmountSky->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableSky->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingSky->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureSky->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedSky->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
}
