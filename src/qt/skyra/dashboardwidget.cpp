...
#include "qt/skyra/dashboardwidget.h"
#include "qt/skyra/forms/ui_dashboardwidget.h"
#include "qt/skyra/sendconfirmdialog.h"
#include "qt/skyra/txrow.h"
...
DashboardWidget::DashboardWidget(SKYRAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSky, "square-chart-sky");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelSky, "text-chart-sky");
    ...
    setCssProperty(ui->labelAmountSky, "text-stake-sky-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &SKYRAGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SKY")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(28, 13, 13);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isSky = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair SKY, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            sky = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalSky += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSky.append(sky);
        chartData->valuesMN.append(mn);

        int max = std::max(sky, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(156, 34, 35));
    set1->setColor(QColor(250, 132, 133));
    ...
    set0->append(chartData->valuesSky);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSky > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountSky, "text-stake-sky");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountSky, "text-stake-sky-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountSky, ui->labelAmountMN});
    ui->labelAmountSky->setText(GUIUtil::formatBalance(chartData->totalSky, nDisplayUnit));
