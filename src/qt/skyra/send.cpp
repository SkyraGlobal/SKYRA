...
#include "qt/skyra/send.h"
#include "qt/skyra/forms/ui_send.h"
#include "qt/skyra/addnewcontactdialog.h"
#include "qt/skyra/qtutils.h"
#include "qt/skyra/sendchangeaddressdialog.h"
#include "qt/skyra/optionbutton.h"
#include "qt/skyra/sendconfirmdialog.h"
#include "qt/skyra/guitransactionsutils.h"
...
SendWidget::SendWidget(SKYRAGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a SKYRA URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-sky");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this]() { onSKYSelected(true); });
    connect(ui->pushRight, &QPushButton::clicked, [this]() { onSKYSelected(false); });
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent SKYs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield SKYs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent SKYs to shield."));
    }
}
...
void SendWidget::onSKYSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto skyAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(skyAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(skyAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(skyAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
