...
#include "qt/skyra/splash.h"
#include "qt/skyra/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/skyra/governancemodel.h"
...
{
    return QCoreApplication::translate("skyra-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in skyra.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in skyra.qrc)
...
/** Class encapsulating SKYRA Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (SKYRAGUI)
...
{
    window = new SKYRAGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &SKYRAGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SKYRAGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(SKYRAGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SKYRAGUI::DEFAULT_WALLET);
        ...
        // SKYRA: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SKYRAGUI::handlePaymentRequest);
        connect(window, &SKYRAGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. SKYRA can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(skyra_locale);
    Q_INIT_RESOURCE(skyra);
    ...
    /// 6. Determine availability of data directory and parse skyra.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", SKYRA_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // skyra: links repeatedly have their payment requests routed to this process:
