...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select SKY Outputs to Spend" :
            "Select Shielded SKY to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::SKY;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show SKYRA address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
