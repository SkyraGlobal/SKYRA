...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SKY);
    ...
    if (settings.contains("nAnonymizeSKYRAAmount") || reset)
        gArgs.SoftSetArg("-anonymizeskyraamount", settings.value("nAnonymizeSKYRAAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in skyra.cpp during shotdown to also remove the window positions
