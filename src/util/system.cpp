...
const char * const SKYRA_CONF_FILENAME = "skyra.conf";
const char * const SKYRA_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "skyrad -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SKYRA
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SKYRA
// Mac: ~/Library/Application Support/SKYRA
// Unix: ~/.skyra
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SKYRA";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SKYRAParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SKYRAParams
    // Mac: ~/Library/Application Support/SKYRAParams
    // Unix: ~/.skyra-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SKYRAParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "SKYRAParams";
#else
    // Unix
    return pathRet / ".skyra-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/skyra/sapling-spend.params";
        sapling_output = "/usr/share/skyra/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/skyra/sapling-spend.params";
            sapling_output = "/usr/local/share/skyra/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", SKYRA_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        // Create empty magnus.conf if it does not exist
