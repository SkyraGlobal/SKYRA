...
// Copyright (c) 2020 The SKYRA Core developers
...
static bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, const CCoinsViewCache* view, CValidationState& state)
{
    ...
    if (pl.collateralOutpoint.hash.IsNull()) {
        ...
        if (pindexPrev != nullptr) {
            if (!CheckCollateralOut(tx.vout[pl.collateralOutpoint.n], pl, state, collateralTxDest, pindexPrev->nHeight + 1)) {
                // pass the state returned by the function above
                return false;
            }
        ...
        CTxDestination collateralTxDest;
        if (!CheckCollateralOut(coin.out, pl, state, collateralTxDest, pindexPrev->nHeight + 1)) {
            // pass the state returned by the function above
            return false;
        }
