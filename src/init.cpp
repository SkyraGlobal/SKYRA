 ...
 */
const char * const SKYRA_PID_FILENAME = "skyra.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", SKYRA_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("skyra-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", SKYRA_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", SKYRA_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-2024 The PIVX Core Developers"))) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2024 The %s Developers"), PACKAGE_NAME)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("skyra-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that SKYRA is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    // Check level must be 4 for checks
...
{
    // Make sure only a single SKYRA process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SKYRA_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if SKYRA is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if SKYRA is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // SKYRA: load previous sessions sporks if we have them.
