...
// static 0.1 SKY fee used for the special txes in these tests
                                          ...
                                          bool fInvalidCollateral = false)
{
    ...
    FundTransaction(tx, utxos, scriptPayout,
                    GetScriptForDestination(coinbaseKey.GetPubKey().GetID()),
                    (optCollateralOut ? 0 : Params().GetConsensus().GetMNCollateral(chainTip->nHeight) - (fInvalidCollateral ? 1 : 0)));
...
BOOST_FIXTURE_TEST_CASE(dip3_protx, TestChain400Setup)
{
    ...
    // Try to register with invalid external collateral
    {
        ...
        CMutableTransaction mtx;
        chainTip = chainActive.Tip();
        const COutPoint& coll_out = CreateNewUTXO(utxos, mtx, coinbaseKey, Params().GetConsensus().GetMNCollateral(chainTip->nHeight) - 1);
        CreateAndProcessBlock({mtx}, coinbaseKey);
        BOOST_CHECK_EQUAL(chainTip->nHeight, ++nHeight);
        Coin coll_coin;
        BOOST_CHECK(view->GetUTXOCoin(coll_out, coll_coin));
        BOOST_CHECK_EQUAL(coll_coin.out.nValue, Params().GetConsensus().GetMNCollateral(chainTip->nHeight) - 1);
    ...
    // Try to register with spent external collateral
    {
        ...
        CMutableTransaction mtx;
        chainTip = chainActive.Tip();
        const COutPoint& coll_out = CreateNewUTXO(utxos, mtx, coinbaseKey, Params().GetConsensus().GetMNCollateral(chainTip->nHeight));
        CreateAndProcessBlock({mtx}, coinbaseKey);
        BOOST_CHECK_EQUAL(chainTip->nHeight, ++nHeight);
        Coin coll_coin;
        BOOST_CHECK(view->GetUTXOCoin(coll_out, coll_coin));
        BOOST_CHECK_EQUAL(coll_coin.out.nValue, Params().GetConsensus().GetMNCollateral(chainTip->nHeight));

        // spend it
        CMutableTransaction spendTx;
        chainTip = chainActive.Tip();
        spendTx.vin.emplace_back(coll_out);
        spendTx.vout.emplace_back(Params().GetConsensus().GetMNCollateral(chainTip->nHeight) - 1000,
                                  GetScriptForDestination(coinbaseKey.GetPubKey().GetID()));
