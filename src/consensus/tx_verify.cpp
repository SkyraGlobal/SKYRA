...
bool CheckTransaction(const CTransaction& tx, CValidationState& state, bool fColdStakingActive)
{
    ...
    static_assert(MAX_TX_SIZE_AFTER_SAPLING > MAX_STANDARD_TX_SIZE, "New max TX size must be bigger than old max TX size");  // sanity
    const unsigned int nMaxSize = tx.IsShieldedTx() ? MAX_TX_SIZE_AFTER_SAPLING : MAX_STANDARD_TX_SIZE;
