...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("SKYRAMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("SKYRAMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SKYRAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("skyra-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SKYRAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("SKYRAMiner exception");
    } catch (...) {
        LogPrintf("SKYRAMiner exception");
    }

    LogPrintf("SKYRAMiner exiting\n");
}
