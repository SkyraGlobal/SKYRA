...
class ReorgStakeTest(SKYRATestFramework):
    ...
    def check_money_supply(self, expected_sky):
        # verify that nodes have the expected SKY supply
        sky_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(sky_supply, [DecimalAmt(expected_sky)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that SKY supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check SKY supply...")
