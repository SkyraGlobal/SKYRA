...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SKYRAParams"
    else
        PARAMS_DIR="$HOME/.skyra-params"
    fi
fi
...
Failed to fetch the SKYRA zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SKYRA - fetch-params.sh

This script will fetch the SKYRA zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SKYRA zkSNARK parameters. Note that it is
