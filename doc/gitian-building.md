...
*Setup instructions for a gitian build of SKYRA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SKYRA
...
to the SKYRA GitHub release page.
...
- [Building SKYRA Core](#building-skyra-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/skyra-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/skyra-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/skyra-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/skyra-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/skyra-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/skyra-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/skyra-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@skyra.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/skyra-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/skyra-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/skyra-osx-*-build.assert
```
